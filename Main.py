...
def send_msg(session, user_id, s):  # функция отправки сообщений
...
def private_crate(text, peer):  # функция создания канала для личных сообщений
    sms = 'Произошла неизвестная ошибка, попробуйте ещё раз или обратитесь к разработчику!'  # сообщение, отправляемое, если функция не вернула одного из стандартных ответов
    if len(text) != 2:
    ...
    send_msg(resend_bot_session, peer, sms)  # отправка сообщения с результатом выполенения команды
